...
- **Voting:** 추천 and 비추천 each once every 7 days per person (counted from your last one to them; they add up). Votes can't be undone. `firestore.rules` enforces both (`voteAction`); a season reset zeroes the tallies but keeps the timers.
